...
data class ProcessableType(
    ...
    var children: List<ProcessableType>,
    ...
    companion object {
        ...
        fun fromResolver(resolver: Resolver, stubsUnitByDefault: Boolean): List<ProcessableType> {
            ...
            return removeDuplicates(processableTypes, mutableSetOf())
        }

        private fun removeDuplicates(
            processableTypes: List<ProcessableType>,
            seen: MutableSet<String>
        ): List<ProcessableType> {
            val result = mutableListOf<ProcessableType>()
            processableTypes.forEach { type ->
                if (seen.add(type.mockClassName.simpleName)) result.add(type)
            }
            result.forEach { type ->
                type.children = removeDuplicates(type.children, seen)
            }
